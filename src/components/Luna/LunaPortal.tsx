...
function LunaPortalContent({ isOpen, onClose }: LunaPortalProps) {
  ...
  const [pendingPrivacyMode, setPendingPrivacyMode] =
    useState<PrivacyMode>("on-the-record");
  ...
  const {
    ...
    onEnd: () => {
      ...
      setTimeout(() => {
        ...
        if (
          currentState.interactionMode === "voice" &&
  ...
  const speak = useCallback(
    (text: string) => {
      ...
      if (speechQueueRef.current === text) {
        console.log(
          "[Luna] Prevented duplicate speech request:",
          text.substring(0, 50)
        );
        return;
      }
  ...
  const { startListening, stopListening, isListening, resetTranscript } =
    useSpeechRecognition({
      onResult: (text, isFinal) => {
        ...
        if (isFinal && text.trim()) {
          ...
          if (currentMode !== "voice") {
            console.log(
              "[Luna] Ignoring voice result because mode is now text"
            );
            return;
          }
      ...
      onError: (error) => {
        ...
        dispatch({
          type: "SET_ERROR",
          payload: "Could not hear you. Please try again.",
        });
      },
    });
  ...
  const startSession = useCallback(
    async (privacyMode?: PrivacyMode) => {
      ...
      const greeting =
        "Hi! I'm Luna, your guide at Lunim Studio. Tell me about your project and I'll help you find the perfect next steps.";
      ...
      dispatch({
        type: "ADD_MESSAGE",
        payload: { role: "luna", content: greeting },
      });
  ...
  const handleUserInput = useCallback(
    async (input: string) => {
      ...
      const previousUserMessages = state.session.messages.filter(
        (m) => m.role === "user"
      ).length;
      const currentUserTurn = previousUserMessages + 1;
      const MAX_USER_TURNS = 4;
      const MIN_USER_TURNS_FOR_PLAN = 2;
      ...
      if (state.session.plan) {
        ...
        dispatch({
          type: "ADD_MESSAGE",
          payload: { role: "luna", content: closingMessage },
        });
      ...
      try {
        ...
        if (!shouldGeneratePlan && recommendedAction === "ask_more") {
          const nextQuestion =
            ...
            (decision?.suggestedFollowUpAngle &&
            decision.suggestedFollowUpAngle.length > 0
          ...
          const intro =
            clarifyData?.questionIntro &&
            clarifyData.questionIntro.trim().length > 0
          ...
          dispatch({
            type: "ADD_MESSAGE",
            payload: { role: "luna", content: questionMessage },
          });
        ...
        if (recommendedAction === "handoff" || shouldNudgeHuman) {
          ...
          intro =
            "Great! Based on what you've shared, here is a plan that could work well.";
        }
        ...
        dispatch({
          type: "ADD_MESSAGE",
          payload: { role: "luna", content: planMessage },
        });
  ...
  const handleReadSummary = useCallback(() => {
    ...
    const fullSummary = `${
      plan.summary
    } Here are your next steps: ${plan.nextSteps
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: "SET_ERROR",
        payload: "Failed to generate PDF. Please try again.",
      });
    }
  }, [state.session]);
